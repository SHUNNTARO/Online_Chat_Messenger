...
class UDP_Server:
    ...
    def handle_message(self) -> None:
        message_bytes, address = self.recv_data()
        if message_bytes and address:
        ...
        if address not in self.client:
            self.clients[address] = Client_info(user_name,client_address)
        self.clients[address].last_response = time.time()

        print(f'{user_name}: {message}')
        self.send(message_bytes, address)

    def sent(self,message_bytes,address) -> None:
        if message_bytes:
            for client in self.clients.values():
                if client.address != address:
                    self.sock.sent(message_bytes, client.address)
                    print('sent {} bytes back to {}'.format(len(message_bytes),address))
